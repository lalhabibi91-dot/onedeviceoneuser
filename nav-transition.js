...
(function () {
  ...
  /**
   * Navigate to `url` instantly — no artificial delay before the browser
   * starts loading the next page. The destination page's own boot script
   * (theme-boot-inline) applies dark mode synchronously before first paint,
   * and its enter animation (_nt-in) provides the polish, so there's no
   * need to hold up navigation here.
   */
  function navigateTo(url) {
    ...
    // Navigate immediately — instant page switch, no waiting on animations.
